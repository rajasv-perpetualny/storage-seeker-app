...
inherits(MarkerLabel_, google.maps.OverlayView);MarkerLabel_.getSharedCross = function (crossURL) {
    ...
    var me = this;var cMouseIsDown = false;var cDraggingLabel = false;var cSavedZIndex;var cLatOffset, cLngOffset;var cIgnoreClick;var cRaiseEnabled;var cStartPosition;var cStartCenter;var cRaiseOffset = 20;var cDraggingCursor = "url(" + this.handCursorURL_ + ")";var cAbortEvent = function cAbortEvent(e) {
        if (e.preventDefault) {
            e.preventDefault();
        }
        ...
        this.getPanes().overlayImage.appendChild(this.crossDiv_);MarkerLabel_.getSharedCross.processed = true;
    }
    this.listeners_ = [google.maps.event.addDomListener(this.eventDiv_, "mouseover", function (e) {
        ...
        cDraggingLabel = false;if (me.marker_.getDraggable()) {
            cMouseIsDown = true;this.style.cursor = cDraggingCursor;
        }
        if (me.marker_.getDraggable() || me.marker_.getClickable()) {
        ...
        var position;if (cMouseIsDown) {
            cMouseIsDown = false;me.eventDiv_.style.cursor = "pointer";google.maps.event.trigger(me.marker_, "mouseup", mEvent);
        }
        if (cDraggingLabel) {
            if (cRaiseEnabled) {
            ...
            me.crossDiv_.style.display = "none";me.marker_.setZIndex(cSavedZIndex);cIgnoreClick = true;cDraggingLabel = false;mEvent.latLng = me.marker_.getPosition();google.maps.event.trigger(me.marker_, "dragend", mEvent);
        }
        ...
        var position;if (cMouseIsDown) {
            if (cDraggingLabel) {
                ...
                cLatOffset = mEvent.latLng.lat() - me.marker_.getPosition().lat();cLngOffset = mEvent.latLng.lng() - me.marker_.getPosition().lng();cSavedZIndex = me.marker_.getZIndex();cStartPosition = me.marker_.getPosition();cStartCenter = me.marker_.getMap().getCenter();cRaiseEnabled = me.marker_.get("raiseOnDrag");cDraggingLabel = true;me.marker_.setZIndex(1000000);mEvent.latLng = me.marker_.getPosition();google.maps.event.trigger(me.marker_, "dragstart", mEvent);
            }
        }
    }), google.maps.event.addDomListener(document, "keydown", function (e) {
        if (cDraggingLabel) {
            if (e.keyCode === 27) {
                cRaiseEnabled = false;me.marker_.setPosition(cStartPosition);me.marker_.getMap().setCenter(cStartCenter);google.maps.event.trigger(document, "mouseup", e);
            }
        }
        ...
        if (me.marker_.getDraggable() || me.marker_.getClickable()) {
            if (cIgnoreClick) {
                ...
                google.maps.event.trigger(me.marker_, "click", e);cAbortEvent(e);
            }
        }
    ...
    var content = this.marker_.get("labelContent");if (typeof content.nodeType === "undefined") {
        ...
        this.labelDiv_.innerHTML = "";this.labelDiv_.appendChild(content);content = content.cloneNode(true);this.eventDiv_.appendChild(content);
    }
...
var createMarkerIcon = function createMarkerIcon(hover, index) {
...
var createMarker = function createMarker(point, index) {
...
var showInfoBox = function showInfoBox(marker, index) {
...
function fetchInfoWindowContent(id) {
    var facility = this.jQuery('#facility-' + id);
    var details = facility.find('.details').clone();
    details.find('.call-now').remove();

    var detailsHtml = details.html();
    var image = facility.find('.image').html();
    return image + detailsHtml;
}

function initFacilityHover() {
    $('body.search li.facility').each(function (i, li) {
        $(li).hover(function () {
            var id = $(li).attr('data-facility-id');
...
var initMaps = function initMaps() {
    ...
    centerPoint = new google.maps.LatLng(parseFloat(this.jQuery('#map-canvas').attr('data-lat')), parseFloat(this.jQuery('#map-canvas').attr('data-lng')));
